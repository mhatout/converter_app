/* global $*/
 ...
 $(document).ready(()=>{
    ...
    summonCurrencies().then(res=>{ 
    		let first_arr = []; let second_arr =[];
	    	...
	    	let currencies = [...first_arr,...second_arr];
    	        populateCurrencies(currencies);});
    $('#curr_submit_btn').on('click',function(e) {
    	e.preventDefault();
        let src_curr = $('#from_curr').attr('value');
        let dest_curr = $('#to_curr').attr('value');
        let amount = $('#from_curr_amount').val();
        let url = `https://free.currencyconverterapi.com/api/v5/convert?q=${src_curr}_${dest_curr}&compact=ultra`;
        $.get(url).then(res=> $('#to_curr_amount').val(round4dec(res[Object.keys(res)[0]]*amount)));
    });
    
    $('#from_curr_dropdown, #to_curr_dropdown').dropdown();
    $('#about_link').css("cursor","pointer").on('click',()=>$('#about_modal').modal('show'));
    $('#contact_link').css("cursor","pointer").on('click',()=>$('#contact_modal').modal('show')); 
    $('#policy_link').css("cursor","pointer").on('click',()=>$('#policy_modal').modal('show'));  
    $("#dropdownItem").dropdown('hide');
    $("#dropdownItem").dropdown({on: 'hover', action: 'hide'});
    $("#dropdownItem").hover(function(){$(this).css("background-color","#f04839");
                                                          $(this).css("color","white"); 
                                                          $(".sync.alternate.icon").addClass("loading");}
                                                    , ()=> $(".sync.alternate.icon").removeClass("loading"));  
    $('#weight_btn').on('click', ()=> {
    ...
    $('#length_btn').on('click', ()=>$("#length_segment").slideToggle());
    $('#capacity_btn').on('click', ()=>$("#capacity_segment").slideToggle());
    $('#pdf_btn').on('click', ()=>$("#pdf_segment").slideToggle());
    ...
    $('#from_pdf_btn').on('click', ()=>{$("#to_pdf_segment, #merge_pdf_segment, .pdf_segment").slideUp(); $("#from_pdf_segment").slideToggle()});
    $('#merge_pdf_btn').on('click', ()=>{$("#to_pdf_segment, #from_pdf_segment, .pdf_segment").slideUp(); $("#merge_pdf_segment").slideToggle()});
    let merge_input_fields =[$('.merge_pdf_inputs')[0]];
    $('#add_merge_input_btn').on('click', ()=>{
    ...
    $('#image_pdf_btn').on('click', ()=>{$('.pdf_segment').slideUp(); $('#image_pdf_segment').slideToggle();});
    $('#word_pdf_btn').on('click', ()=>{$('.pdf_segment').slideUp(); $('#word_pdf_segment').slideToggle();});
    $('#excel_pdf_btn').on('click', ()=>{$('.pdf_segment').slideUp(); $('#excel_pdf_segment').slideToggle();});
    $('#power_pdf_btn').on('click', ()=>{$('.pdf_segment').slideUp(); $('#power_pdf_segment').slideToggle();});
    
    $('#pdf_image_btn').on('click', ()=>{$('.pdf_segment').slideUp(); $('#pdf_image_segment').slideToggle();});
    $('#pdf_word_btn').on('click', ()=>{$('.pdf_segment').slideUp(); $('#pdf_word_segment').slideToggle();});
    $('#pdf_excel_btn').on('click', ()=>{$('.pdf_segment').slideUp(); $('#pdf_excel_segment').slideToggle();});
    $('#pdf_power_btn').on('click', ()=>{$('.pdf_segment').slideUp(); $('#pdf_power_segment').slideToggle();});
    ...
    $('#image_pdf_submit_btn').on('click', function(e){
        ...
        let str = $('#image_pdf_input')[0].files[0].name;    
    ...
    $('#word_pdf_submit_btn').on('click', function(e){
        ...
        let str = $('#word_pdf_input')[0].files[0].name;    
        let name = str.split(".")[0];    
            $.post({
    ...
    $('#excel_pdf_submit_btn').on('click', function(e){
        ...
        let str = $('#excel_pdf_input')[0].files[0].name;    
        let name = str.split(".")[0];    
            $.post({
    ...
    $('#pdf_image_submit_btn').on('click', function(e){
        ...
        let str = $('#pdf_image_input')[0].files[0].name;    
        let name = str.split(".")[0];    
            $.post({
    ...
    $('#pdf_word_submit_btn').on('click', function(e){
        ...
        let str = $('#pdf_word_input')[0].files[0].name;    
        let name = str.split(".")[0];    
            $.post({
    ...
    $('#pdf_excel_submit_btn').on('click', function(e){
        ...
        let str = $('#pdf_excel_input')[0].files[0].name;    
        let name = str.split(".")[0];    
            $.post({
    ...
    $('#pdf_power_submit_btn').on('click', function(e){
        ...
        let str = $('#pdf_power_input')[0].files[0].name;    
        let name = str.split(".")[0];    
            $.post({
        ...
        $('#merge_pdf_submit_btn').on('click', function(e){
    	    e.preventDefault();
        let formData = new FormData();
        merge_input_fields.forEach((val, i) =>{
            if (val.files[0]){
            formData.append(`Files[${i}]`, val.files[0], val.files[0].name);
            }
           });
        let str = merge_input_fields[0].files[0].name;    
        let name = str.split(".")[0];    
            $.post({
    ...
    $('#temp_btn').on('click', ()=>$('#temp_modal').modal('show'));
    $('#date_btn').on('click', ()=>$('#date_modal').modal('show'));
    $('#CDay').on('keyup', function() {moveOnMax($(this), $('#CMonth'))});
    $('#CMonth').on('keyup', function() {moveOnMax($(this), $('#CYear'))});
    $('#CYear').on('keyup', function() {chrToIsl()});
    $('#HYear').on('keyup', function() {islToChr()});
    $('#HDay').on('keyup', function() {moveOnMax($(this), $('#HMonth'))});
    $('#HMonth').on('keyup', function() {moveOnMax($(this), $('#HYear'))});
    $('#reset').on('click', ()=>$('.date').val(''));
...
function handle_weight_avoir_input(input, num){
    ...
    function long_ton(l_ton){
		...
		function grain_voir(grain){
			let grain_voir = parseFloat(grain);
...
function handle_length_input(input, num){
    ...
    function kilometer(num){
        let standard_unit = round4dec(parseFloat(num)*1000);
        standrize_and_distrebute(standard_unit);
    }
    
    function meter(num){
        let standard_unit = round4dec(parseFloat(num)*1);
        standrize_and_distrebute(standard_unit);
    }
    
    function decimeter(num){
    	let standard_unit = round4dec(parseFloat(num)*0.1);
        standrize_and_distrebute(standard_unit);
    }
    
    function centimeter(num){
    	let standard_unit = round4dec(parseFloat(num)*0.01);
        standrize_and_distrebute(standard_unit);
    }
    
    function millimeter(num){
    	let standard_unit = round4dec(parseFloat(num)*0.001);
        standrize_and_distrebute(standard_unit);
    }
    
    function micrometer(num){
    	let standard_unit = parseFloat(num)*1.0E-6;
        standrize_and_distrebute(standard_unit);
    }
    
    function Nanometer(num){
    	let standard_unit = round4dec(parseFloat(num)*1.0E-9);
        standrize_and_distrebute(standard_unit);
    }
    
    function angstrom(num){
    	let standard_unit = round4dec(parseFloat(num)*1.0E-10);
        standrize_and_distrebute(standard_unit);
    }
    
    function league(num){
    	let standard_unit = round4dec(parseFloat(num)*4828);
        standrize_and_distrebute(standard_unit);
    }
    
    function mile(num){
    	let standard_unit = round4dec(parseFloat(num)*1609);
        standrize_and_distrebute(standard_unit);
    }
    ...
    function bolt(num){
    	let standard_unit = round4dec(parseFloat(num)*36.58);
        standrize_and_distrebute(standard_unit);
    }
    
    function foot(num){
    	let standard_unit = round4dec(parseFloat(num)*0.3048);
        standrize_and_distrebute(standard_unit);
    }
    ...
    function span(num){
    	let standard_unit = round4dec(parseFloat(num)*0.2286);
        standrize_and_distrebute(standard_unit);
    }
    
    function hand(num){
    	let standard_unit = round4dec(parseFloat(num)*0.1016);
        standrize_and_distrebute(standard_unit);
    }
    ...
    function nautical_league(num){
    	let standard_unit = round4dec(parseFloat(num)*5556);
        standrize_and_distrebute(standard_unit);
    }
    
    function nautical_mile(num){
    	let standard_unit = round4dec(parseFloat(num)*1852);
        standrize_and_distrebute(standard_unit);
    }
    
    function cable_length(num){
    	let standard_unit = round4dec(parseFloat(num)*185.2);
        standrize_and_distrebute(standard_unit);
    }
    
    function fathom(num){
    	let standard_unit = round4dec(parseFloat(num)*1.829);
        standrize_and_distrebute(standard_unit);
    }
    
    function us_nautical_mile(num){
    	let standard_unit = round4dec(parseFloat(num)*1853);
        standrize_and_distrebute(standard_unit);
    }
    ...
    function us_cable_length(num){
    	let standard_unit = round4dec(parseFloat(num)*219.5);
        standrize_and_distrebute(standard_unit);
    }
    ...
    function admiralty_cable_length(num){
    	let standard_unit = round4dec(parseFloat(num)*185.3);
        standrize_and_distrebute(standard_unit);
    }
    
    
    function standrize_and_distrebute(unit){
        let convert_to_kilometer = unit*0.001;
		$('#kilometer').val(convert_to_kilometer);
		let convert_to_meter = unit*1;
		$('#meter').val(convert_to_meter);
        let convert_to_decimeter = unit*10;
		$('#decimeter').val(convert_to_decimeter);		  
        let convert_to_centimeter = unit*100;
		$('#centimeter').val(convert_to_centimeter);		  
        let convert_to_milimeter = unit*1000;
		$('#millimeter').val(convert_to_milimeter);		  
        let convert_to_micro = unit*1000000;
		$('#micrometer').val(convert_to_micro);		 
        let convert_to_nano = unit*1000000000;
		$('#Nanometer').val(convert_to_nano);			  
        let convert_to_angstrom = unit*10000000000;
		$('#angstrom').val(convert_to_angstrom);			  
        let convert_to_aleague = unit*0.00020714;
		$('#league').val(convert_to_aleague);			  
        let convert_to_amile = unit*0.00062144;
		$('#mile').val(convert_to_amile);			  
        let convert_to_aland = unit*0.00062144;
		$('#land').val(convert_to_aland);			  
        let convert_to_abolt = unit*0.027344;
		$('#bolt').val(convert_to_abolt);	
        let convert_to_afoot = unit*3.2814;
	    $('#foot').val(convert_to_afoot);
        let convert_to_afeet = unit*3.2804;
	    $('#feet').val(convert_to_afeet);
        let convert_to_aspan = unit*4.37445;
	    $('#span').val(convert_to_aspan);
        let convert_to_ahand = unit*9.8434;
	    $('#hand').val(convert_to_ahand);
        let convert_to_bleague = unit*0.00020714;
	    $('#league_british').val(convert_to_bleague);
        let convert_to_bmile = unit*0.00062144;
	    $('#mile_british').val(convert_to_bmile);
        let convert_to_bland = unit*0.00062144;
	    $('#land_british').val(convert_to_bland);
        let convert_to_bfoot = unit*3.2814;
	    $('#foot_british').val(convert_to_bfoot);
        let convert_to_bspan = unit*4.37445;
	    $('#span_british').val(convert_to_bspan);
        let convert_to_bnail = unit*4.3744;
        $('#nail').val(convert_to_bnail);
        let convert_to_nleague = unit*0.000184;
        $('#nautical_league').val(convert_to_nleague);
        let convert_to_nmile = unit*0.00544;
        $('#nautical_mile').val(convert_to_nmile);
        let convert_to_ncablelength = unit*0.000544;
        $('#cable_length').val(convert_to_ncablelength);
        let convert_to_nfathom = unit*0.54684;
        $('#fathom').val(convert_to_nfathom);
        let convert_to_usnmile = unit*0.00053964;
        $('#us_nautical_mile').val(convert_to_usnmile);
        let convert_to_usfathom = unit*0.54684;
        $('#us_fathom').val(convert_to_usfathom);
        let convert_to_uscablelength = unit*0.0045574;
        $('#us_cable_length').val(convert_to_uscablelength);
        let convert_to_admiraltymile = unit*0.00053964;
        $('#admiralty_mile').val(convert_to_admiraltymile); 
        let convert_to_admiraltycablelength = unit*0.0053964;
...
function handle_capacity_liquid_input(input, num){
	...
	{
		let beer_barrel = parseFloat(num);  
...
function handle_temp_conv_input(input, num){
    ...
    function celzz(celsi) {
		  let convert_cel_kel = round4dec(parseFloat(celsi)+273.15);
		...
		function kelzz(kelv) {
		  let convert_kel_cel = round4dec(parseFloat(kelv)-273.15);
...
function moveOnMax(field, nextFieldID) { if (field.val().length >= field.attr("maxlength")) { nextFieldID.focus() } }
...
function chrToIsl() {
	let d =parseInt($('#CDay').val());
	let m =parseInt($('#CMonth').val());
	let y =parseInt($('#CYear').val());
	let jd;
...
function islToChr() {
	let d = parseInt($('#HDay').val());
	let m = parseInt($('#HMonth').val());
	let y = parseInt($('#HYear').val());
	let jd = intPart((11*y+3)/30)+354*y+30*m-intPart((m-1)/2)+d+1948440-385;
	$('#JD').val(jd);
	$('#wd').val(weekDay(jd%7));
					if (jd> 2299160 )
						{
					  ...
					  let j=intPart((80*l)/2447);
						...
						{
					  let j=jd+1402;
					  let k=intPart((j-1)/1461);
					  let l=j-1461*k;
					  let n=intPart((l-1)/365)-intPart(l/1461);
					  let i=l-365*n+30;
						  j=intPart((80*i)/2447);
...
async function summonCurrencies(){
	...
	let currencies = [];
	await $.get('https://free.currencyconverterapi.com/api/v5/currencies').then(res=> Object.keys(res.results).forEach(key =>currencies.push(res.results[key])));
	return currencies;
...
function populateCurrencies(arr){
	...
	arr.forEach(element=> $('.curr_menu').append(`<div class="item" data-value="${element.id}">${element.id} - ${element.currencyName}</div>`));
